	...
	<section class="profile-mein-content">
		<div class="container">
            ...
            if (isset($_POST['upss'])){
                ...
                if (password_verify($old_pass, $_SESSION['password'])==false){
                    $ops_chk = false;
                }else{
                    $ops_chk = true;
                }
                ...
                if (empty($old_pass)|| empty($new_pass) || empty($confirm_pass)){
                    ...
                    $mess = msgvld('New password are not match with confirm password', 'warning');
                }elseif ($ops_chk==false){
                    $mess = msgvld('Old password are not match with your main password', 'warning');
                }else{
                    $usr_id= $_SESSION['id'];
                    $mk_nw_pass = password_hash($new_pass, PASSWORD_DEFAULT);
                    $qry = "UPDATE users SET password = '$mk_nw_pass' WHERE id = '$usr_id'";
                    $con->query($qry);
                    $mess = msgvld('Password change done', 'success');
                }
            }
            ?>
